    ...
    if os.path.exists(csv_path):
        if Product.query.count() == 0:
            ...
            with open(csv_path, 'r') as f:
                ...
                for row in reader:
                    try:
                        # Use CSV ID if available, ensuring unique
                        p_id = int(row['id'])
...
@login_required
def inventory():
    # Pass 'products' as 'items' to match your HTML loop
    all_products = Product.query.all()
    return render_template('inventory.html', items=all_products)
...
if __name__ == '__main__':
    with app.app_context():
        db.create_all()
        load_data_from_csv() # AUTO-LOAD DATA ON START
    app.run(debug=True)